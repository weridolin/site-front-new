<template>
  <ul style="padding: 0">
    ...
    >
      <div
        class="mess-info"
        :class="car.left ? 'mess-info-left' : 'mess-info-right'"
      >
        ...
        <div class="mess-header" :class="{ 'mess-header-dis': !car.pattern }">
          <div class="header-left a-tag">
            ...
            <div v-if="car.pattern" class="header-right">
              ...
              <p class="time">
                {{ getDateDiff(item.createdTimestamps)
                }}<span v-if="item.address"
                  >—<span v-html="formatReply(item.address)"></span
                ></span>
              </p>
            </div>
     ...
     <span class="open" @click="id=id==item.id?0:item.id"><i :class="id==item.id?'el-icon-arrow-up':'el-icon-arrow-down'"></i></span>
        ...
        <div class="mess-footer" v-show="item.replays">
          <div
            class="mess-info mess-reply"
            v-for="reply of item.replays"
            :key="'reply' + reply.id"
            :id="'reply' + reply.id"
          ...
          >
            ...
            <el-popconfirm
              v-if="
                (userInfo.id == reply.user.id && userInfo.id > 0) ||
                userInfo.is_admin == 1
              "
              title="这条留言确认删除吗？"
              @onConfirm="dele(reply.user.id, 'reply')"
            >
              <a href="javascript:;" class="dele-reply el-icon-delete"></a>
            </el-popconfirm>
            ...
            <div class="mess-header">
              <div class="header-left a-tag">
                ...
                <div class="header-right">
                  <a href="javascript:;" class="el-icon-sugar mess-name"
                    >{{ reply.name }}
                  </a>
                  回复
                  <a href="javascript:;" class="el-icon-sugar mess-name">{{
                  ...
                  >
                  <p>
                    {{ getDateDiff(reply.createdTimestamps)
                    }}<span v-if="reply.address"
                      >—<span v-html="reply.address"></span
                    ></span>
                  </p>
                </div>
                <a
...
export default {
  ...
  data() {
    return {
      ...
      reply_id: 0,
      // mess_reply_id:'',
      url: "https://ss0.bdstatic.com/70cFvHSh_Q1YnxGkpoWK1HF6hhy/it/u=3656238279,115253853&fm=26&gp=0.jpg",
      boo: false,
      FormData: {
        mess_id: "",
        reply: "",
