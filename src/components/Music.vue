  ...
  methods: {
    ...
    getMusic() {
      if (!this.musics[this.index].pause) {
        console.log("获取音乐",this.musics[this.index]);
        const qs = require("qs");
        let that = this;
        ...
        this.musicInfo = {
          ...
          pic: this.musics[this.index].avatar,
    ...
    getList() {
      let that = this;
      this.$get("/api/v1/home/bglist")
        .then(function (res) {
          // console.log(">>> get music",res.data)
    ...
    mEnded() {
      console.log(">>>播放完成下一首INDEX",this.cycle)
      if (this.cycle == 0) {
        this.audio.pause(); // 暂停
        this.isPlay = false;
